...
} from '@neup/logica/account/token/authenticate';
import { verifyNeupIdToken } from '@neup/logica/account/token/verify';
...
async function getServerAuthAccountToken(): Promise<string | null> {
  ...
  try {
    const { getCookie } = await import('@neup/core/helpers/cookie');
